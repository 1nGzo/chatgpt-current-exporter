(function () {
  ...
  function formatDiagnostics(snapshot) {
    ...
    return [
      ...
      `Conversation Title: ${d.conversationTitle || (snapshot && snapshot.title) || "—"}`,
      `Fetch observed: ${d.fetchObserved || 0}`,
      `XHR observed: ${d.xhrObserved || 0}`,
      `Streaming responses: ${d.streamResponses || 0}`,
  ...
  function show(snapshot) {
    ...
    if (statusContainer && statusContainer.dataset) statusContainer.dataset.state = s.toLowerCase();
    state.style.color = "";
